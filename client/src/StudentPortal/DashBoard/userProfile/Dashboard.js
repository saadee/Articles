							...
							{posts.map((post) => (
								<TableRow key={post.name}>
									<TableCell component="th" scope="post">
										<img
											src={post.length ? post.titleImg : post.titleImg}
											alt=""
											style={{ width: '50px' }}
										/>
									</TableCell>
									...
									<TableCell align="center">
										<Link to={`/article/edit/${post.length ? post._id : post._id}`}>
											<CreateIcon />
										</Link>
									</TableCell>{' '}
									<TableCell align="center">
										<span style={{ fontWeight: 'bold' }}>{new Date(post.date).toDateString()}</span>{' '}
										<span></span>
										<span></span>
										<input
											type="date"
